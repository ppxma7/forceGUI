function metrics = computeTRAP(forceFilt, fs, extra, covWin)
% forceFilt = filtered force segment (app.forcewave)
% fs        = sampling frequency
% extra     = if you have a multichannel file, then get xcorr between left
%             and right force traces
% covWin    = option for user to input their best CoV of force traces.
%             Default is 2 and 5s
...
if isempty(covWin)
    ...
    %window_times = covWin;
    window_times = str2num(covWin); %#ok<ST2NM>
end
...
% Make this generic
% Per-window metrics (dynamic field names)
for w = 1:numel(window_times)
    t = window_times(w);
    fname = num2str(t);  % e.g. '5', '2', '10'
    if ~isempty(lowest_cov_data{w})
        metrics.(sprintf('mean_%ss', fname)) = mean(lowest_cov_data{w});
        metrics.(sprintf('std_%ss',  fname)) = std(lowest_cov_data{w});
        metrics.(sprintf('CoV_%ss',  fname)) = metrics.(sprintf('std_%ss', fname)) / ...
                                               metrics.(sprintf('mean_%ss', fname)) * 100;
    else
        metrics.(sprintf('mean_%ss', fname)) = NaN;
        metrics.(sprintf('std_%ss',  fname)) = NaN;
        metrics.(sprintf('CoV_%ss',  fname)) = NaN;
    end
end
